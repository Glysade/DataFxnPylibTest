...
class ScriptTest(TestCase):
    ...
    def setUp(self) -> None:
        ...
        script_file = this_dir / 'QEDScore_script_dev.py'
        if Path(script_file).exists():
            if not PRINTED_GUFF:
                print(f'Using development script')
                PRINTED_GUFF = True
            from QEDScore_script_dev import execute
            ...
            from QEDScore_script import execute
    ...
    def test_script(self) -> None:
        file_in = Path(__file__).parent / 'resources' / 'test_qed1.json'
